...
async function ensureProfile(supabase, userId, email) {
    ...
    const { data: existing } = await supabase.from("profiles").select("display_name").eq("id", userId).single();
    if (!existing) {
...
async function InvitePage({ params }) {
    ...
    if (!user) {
        (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$client$2f$components$2f$navigation$2e$react$2d$server$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["redirect"])(`/login?next=${encodeURIComponent(`/invite/${code}`)}`);
    }
    await ensureProfile(supabase, user.id, user.email);
    const shortCode = code.trim().toLowerCase();
    const { data: game, error: gameError } = await supabase.from("games").select("id, status").eq("short_code", shortCode).single();
    if (gameError || !game || game.status !== "active") {
        (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$client$2f$components$2f$navigation$2e$react$2d$server$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["notFound"])();
    }
    const { data: existingParticipation } = await supabase.from("game_players").select("status").eq("game_id", game.id).eq("user_id", user.id).maybeSingle();
    ...
    (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$client$2f$components$2f$navigation$2e$react$2d$server$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["redirect"])(`/game/${game.id}`);
}
}),
...
//# sourceMappingURL=%5Broot-of-the-server%5D__07bc17e0._.js.map