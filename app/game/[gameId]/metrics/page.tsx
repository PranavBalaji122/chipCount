...
export default async function GameMetricsPage({
  ...
  const [
    ...
    supabase
      .from("game_players")
      .select(
        "user_id, cash_in, cash_out, profile:profiles(display_name, venmo_handle)",
      )
      .eq("game_id", gameId)
      .eq("status", "approved"),
    supabase
  ...
  const mySnapshots = allSnapshots.filter(
  ...
  for (const s of allSnapshots) {
    snapshotTotals[s.user_id] =
      (snapshotTotals[s.user_id] ?? 0) + Number(s.session_net);
  }

  const sessionPlayers = (approvedPlayers ?? []).map(
    (p: {
      user_id: string;
      cash_in: number | null;
